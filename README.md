...
* a clap-based CLI with stable top-level commands, global flags, configuration discovery, profiles, human/JSON/NDJSON/quiet output renderers, agent registry commands, public A2A Agent Card discovery/cache inspection, and A2A interface negotiation for inspected cards
...
See [`docs/cli.md`](docs/cli.md) for current command behaviour and the output envelope, [`docs/configuration.md`](docs/configuration.md) for config discovery, schema, state paths, examples, validation, and redaction, [`docs/protocol.md`](docs/protocol.md) for the current Agent Card discovery and interface negotiation mapping, and [`docs/storage.md`](docs/storage.md) for the SQLite migration/schema contract. Future tickets add official/vendored A2A type integration, message sending, streaming, gateway runtime behaviour, adapters, collectives, tests, and packaging.
