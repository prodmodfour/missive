...
See [`docs/cli.md`](docs/cli.md) for current command behaviour and the output envelope, [`docs/collectives.md`](docs/collectives.md) for the implemented broadcast, barrier, gather, and reduce collectives, [`docs/configuration.md`](docs/configuration.md) for config discovery, schema, A2A service-parameter/auth/routing defaults, state paths, examples, validation, and redaction, [`docs/protocol.md`](docs/protocol.md) for the current official A2A type boundary, Agent Card discovery, service-parameter/auth handling, send/stream/task subscription/push/webhook/artifact mapping, context continuity, interface negotiation mapping, and conformance fixture coverage, [`docs/storage.md`](docs/storage.md) for the SQLite migration/schema contract, [`docs/gateway.md`](docs/gateway.md) for gateway daemon operation, subscription resume, background jobs, service installation, and log inspection, [`docs/adapters.md`](docs/adapters.md) for the adapter trait, registry, lifecycle, and gateway event-bus bridge, [`docs/security.md`](docs/security.md) for auth storage tradeoffs, and [`docs/testing.md`](docs/testing.md) for local validation, reusable mock A2A fixtures, gateway/webhook/job integration coverage, capability-selection tests, and protocol-versioned conformance fixtures. Future tickets add embedded webhook management, concrete adapters, broader tests, and packaging.
