...
`crates/missive-a2a` currently owns the public Agent Card discovery client and compatibility structs for the A2A AgentCard fields needed by inspection: provider, versions, capabilities, supported interfaces, default media modes, and skills. The client resolves `/.well-known/agent-card.json` from the registered base URL, uses `reqwest` with rustls TLS, preserves ETag/Last-Modified validators, parses the raw JSON into a typed Agent Card summary, and maps malformed card JSON to protocol errors while mapping HTTP/TLS/network failures to transport errors. Full official/vendored protocol type integration remains a later ticket.
