...
`crates/missive-cli` owns the clap-derived `Cli`, `GlobalArgs`, and `Commands` types. The CLI currently exposes top-level commands for `agent`, `send`, `stream`, `task`, `context`, `group`, `gateway`, `webhook`, `push`, `doctor`, `logs`, `events`, `completion`, and `manpage`. The `agent` command has implemented registry subcommands for `add`, `remove`, `list`, `show`, `inspect`, `refresh`, and `rename`; `send` now performs non-streaming A2A `SendMessage` calls; other top-level commands remain skeletal until their ordered tickets land.

Global flags are parsed at every command level: `--json`, `--ndjson`, `--quiet`, `--no-color`, `--config`, `--profile`, `--timeout`, `--protocol-version`, `--a2a-extension`, `--service-param`, `--bearer-token-env`, `--header`, `--trace`, and `--verbose`. `--config`, `--profile`, A2A service-parameter flags, and auth header flags now feed implemented Agent Card and send requests; timeout enforcement and tracing remain for later tickets.
