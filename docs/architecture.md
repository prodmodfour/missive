...
`crates/missive-cli` owns the clap-derived `Cli`, `GlobalArgs`, and `Commands` types. The CLI currently exposes top-level commands for `agent`, `send`, `stream`, `task`, `context`, `group`, `gateway`, `webhook`, `push`, `doctor`, `logs`, `events`, `completion`, and `manpage`. The `agent` command now has implemented registry subcommands for `add`, `remove`, `list`, `show`, `inspect`, `refresh`, and `rename`; other top-level commands remain skeletal until their ordered tickets land.
...
`crates/missive-a2a` currently owns the public Agent Card discovery client, compatibility structs for the A2A AgentCard fields needed by inspection, and the first interface negotiation helper. The client resolves `/.well-known/agent-card.json` from the registered base URL, uses `reqwest` with rustls TLS, preserves ETag/Last-Modified validators, parses the raw JSON into a typed Agent Card summary, and maps malformed card JSON to protocol errors while mapping HTTP/TLS/network failures to transport errors. Interface negotiation canonicalizes Agent Card bindings such as `HTTP+JSON` and `JSONRPC` to missive's `http+json`/`json-rpc` names, respects the agent row's ordered preference or an explicit CLI `--binding` override, recognizes gRPC as a future extension point, and falls back to registry/base URL interfaces for older cards that omit `supportedInterfaces`. Full official/vendored protocol type integration remains a later ticket.
