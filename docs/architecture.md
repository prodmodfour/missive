...
Global flags are parsed at every command level: `--json`, `--ndjson`, `--quiet`, `--no-color`, `--config`, `--profile`, `--timeout`, `--protocol-version`, `--a2a-extension`, `--service-param`, `--trace`, and `--verbose`. `--config`, `--profile`, and A2A service-parameter flags now feed implemented Agent Card requests; timeout enforcement and tracing remain for later tickets.
