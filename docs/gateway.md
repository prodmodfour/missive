...
Gateway foreground and service logs use the shared missive logging foundation. Set `RUST_LOG=info` or a more specific EnvFilter such as `RUST_LOG=missive_gateway=debug` before `missive gateway run`; set `MISSIVE_LOG_FORMAT=json` when service managers or log collectors should receive one JSON log object per stderr line. Log rendering redacts authorization headers, cookies, tokens, and secret-like fields before writing to stderr. Use `missive logs --json` to inventory the selected profile's profile-file, event-journal, and gateway-service diagnostic sources; use `missive logs --source profile-files --ndjson` when a local supervisor redirects gateway stderr into `<profile state dir>/logs/`. See [`observability.md`](observability.md) for the exact filter precedence, JSON mode, destinations, and redaction boundaries.
...
No configured stdio/file-drop/external adapter is started from config by `gateway run` yet; the bridge is tested with fake adapter events and the opt-in HTTP adapter. The HTTP adapter can be mounted with `--http-adapter` and emits accepted control frames as `gateway_adapter_event` runtime output. The concrete stdio adapter is available today as the foreground `missive adapter stdio` subprocess loop, and the concrete file-drop adapter is available as the foreground `missive adapter file-drop` inbox/outbox loop. Discord, Slack, Telegram, Matrix, and Email adapter kinds are feature-gated stubs in the adapter crate only; they expose registry/identity placeholders but no live platform clients. See [`adapters.md`](adapters.md) for lifecycle, frame, file handoff, HTTP control-frame details, and external stub boundaries.
