...
cargo clippy -p missive-store --all-targets -- -D warnings
cargo check -p missive-cli --all-targets --no-default-features
scripts/quality-gate.sh
```

The targeted checks covered typed artifact repository CRUD/cascade behavior, task artifact list/show/save/export output, safe artifact filename sanitization, multiple artifacts returned in a remote task, incremental streaming artifact updates, updated help snapshots, no-default CLI compilation, and CLI/store regression coverage.
...
Implemented ticket 024 — Implement artifact handling and export.

Included:

* added typed artifact repository APIs in `missive-store` with `ArtifactId`, `ArtifactKind`, `ArtifactUpsert`, `ArtifactRecord`, CRUD/list methods, task-scoped listing, and transaction support
* persisted artifacts embedded in A2A `Task` payloads returned by send, stream task events, task get/list/wait, and task cancel
* persisted streaming `artifactUpdate` events to dedicated artifact rows, including append-chunk merging and local version increments
* added `missive task artifact list/show/save/export` with human, JSON, NDJSON, and quiet output support
* rendered artifact metadata summaries in task show/list/wait/cancel output where dedicated artifact rows are available
* implemented safe disk export for text, JSON data, raw-byte, and URL/file-reference artifacts; remote names/filenames are sanitized and exports refuse overwrites unless `--force` is supplied
* added CLI and store tests for multiple returned artifacts, incremental artifact updates, artifact metadata rendering, safe path handling, and task artifact save/export behavior
* updated task help snapshots plus README and CLI, protocol, storage, architecture, and security docs
...
The `missive` binary has a real command tree, global parser, configuration discovery/profile validation, A2A service-parameter flags, auth input flags, output rendering contract, implemented agent registry commands, Agent Card inspection/refresh, selected-interface negotiation, non-streaming send, streaming send, task get/list/wait/cancel, task artifact list/show/save/export, and context create/list/show/fork/close/export. Gateway behaviour, adapters, collectives, shell completion generation, and manpage generation remain for later tickets.

The store layer resolves state paths, provides process locks, migrates fresh SQLite databases to schema version 1, and exposes typed repository APIs for auth refs, agents, contexts, tasks, artifacts, messages, events, groups, group members, and gateway jobs. Push-config and adapter-binding repositories, retention enforcement, compaction, event replay commands, and broader durable A2A protocol persistence remain for later tickets.
...
Ticket 025 — Implement event journal and replay.
