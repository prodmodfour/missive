...
```bash
cargo test -p missive-store repository::tests::push_config_crud_round_trip_soft_deletes
cargo test -p missive-cli --test push_command --all-features
cargo test -p missive-cli --test help_snapshots --all-features
cargo clippy -p missive-a2a --all-targets --all-features -- -D warnings
cargo clippy -p missive-cli --test push_command --all-features -- -D warnings
...
The targeted checks covered the new push-config repository APIs, CLI `push create/get/list/delete` integration tests against the reusable mock A2A server for HTTP+JSON and JSON-RPC bindings, callback URL validation, callback auth redaction in output/events/store rows, push help snapshots, and workspace clippy/tests before the full quality gate.
...
Implemented ticket 028 — Implement push notification config commands.

Included:

* added `missive push create/get/list/delete` with callback URL validation, optional `--config-id`, callback `--auth-scheme` plus `--auth-credentials-env`, repeatable local `--metadata`, JSON/NDJSON/human output, and A2A service-parameter/outbound auth-header reuse
* added `PushConfigClient` in `missive-a2a` for HTTP+JSON and JSON-RPC Create/Get/List/Delete task push notification config calls using official A2A SDK types where available
* added typed `PushConfigId`, `PushConfigUpsert`, `PushConfigRecord`, and store repository methods for active upserts, list/get, task-scoped listing, and soft deletion via `deleted_at`
* persisted redacted local push-config records and redacted `a2a.push.*` events, creating placeholder task rows when a push command references a task before it has been fetched
* added CLI integration tests against `missive-test-support` covering HTTP+JSON and JSON-RPC push endpoints, all four push commands, callback URL validation, auth credential redaction, event persistence, and local soft-delete state
* added a push help snapshot and updated README plus CLI/protocol/storage/security/testing/architecture docs for implemented push behavior and limitations
...
`missive task` implements direct task get/list/wait/cancel only. It does not implement A2A task resubscription/`SubscribeToTask`, gateway-managed task monitoring, background waits, restart resume, or push/webhook-driven task updates; those remain for later gateway/webhook tickets.
...
`missive push` configures remote A2A task push notification configs and persists redacted local records, but it does not run a webhook receiver, expose local tunnel helpers, validate inbound callback payloads, rotate callback credentials, auto-refresh push configs in the background, or use push events to update task state yet. Those behaviors remain for the webhook and gateway tickets. `push list` is scoped to one task/agent response page and records returned `nextPageToken`, but it does not auto-page through every remote config.
...
Authentication is wired into implemented Agent Card fetch/refresh, non-streaming send, streaming send, remote task, and push config HTTP paths. Webhook/gateway behavior, adapters, collectives, and observability remain for later tickets and must reuse the same auth/redaction path when they add network calls.
...
Ticket 029 — Implement webhook receiver.
