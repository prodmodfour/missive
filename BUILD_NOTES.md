...
Tickets 000 through 013 are complete. The repository uses the target Cargo workspace layout for `missive` with these crates:

* `crates/missive-cli` — package `missive-cli`, binary `missive`, clap-derived CLI tree, global flags, configuration loading from CLI/env/discovery, output rendering contract, redaction helpers, help snapshots, implemented `missive agent add/list/show/remove/rename`, and placeholder execution status for later commands
...
The root `Cargo.toml` is a virtual workspace manifest with shared workspace package metadata and shared dependency versions for planned foundational Rust crates. The store layer depends on `rusqlite` with bundled SQLite plus `serde`/`serde_json` for typed JSON repository boundaries. The CLI now directly depends on the workspace `url` crate for registry URL validation.
...
Architecture decision records live under `docs/adr/`, with a template and initial accepted ADRs for Rust workspace structure, A2A-first protocol strategy, SQLite local state, and CLI-first UX. `docs/architecture.md` links the ADRs and records the current high-level crate boundaries, shared error handling contract, core primitive contract, CLI command/agent-registry contract, configuration contract, output rendering contract, store path/lock contract, SQLite migration contract, and typed repository contract.
...
cargo test -p missive-store --all-targets --all-features
cargo test -p missive-cli --test agent_registry --all-features
cargo fmt --all -- --check
cargo test --workspace --all-targets --all-features
cargo clippy --workspace --all-targets --all-features -- -D warnings
```

The targeted checks covered CLI parsing for nested agent subcommands, help snapshot updates, URL/metadata parsing, non-secret auth-ref persistence, JSON and human agent registry CRUD, duplicate aliases, missing agents, invalid aliases, rename transactions, config-seeded read-only agents, output envelope stability, and workspace regression coverage.

Environment/tooling notes: no new cargo subcommands or OS packages were installed during this cycle. `Cargo.lock` records `url` as a direct dependency of `missive-cli`; the crate version was already managed in the workspace dependency set.

## Latest cycle notes

Implemented ticket 013 — Implement agent registry commands.

Included:

* added `crates/missive-cli/src/agent.rs` with `missive agent add/remove/list/show/rename`
* added SQLite-backed profile registry execution with state path resolution, directory creation, state mutation locking, store migration/opening, config auth-ref sync, and config-seeded read-only agent sync
* added CLI support for aliases, base URLs, explicit `--interface BINDING=URL` entries, `--binding-preference`, `--auth-ref`, repeated `--tag`, `--notes`, and repeated `--metadata KEY=VALUE`
* added URL validation that rejects non-HTTP(S), whitespace/control characters, and embedded credentials
* added duplicate-alias, missing-agent, invalid-alias, read-only config-seed, and same-alias rename diagnostics through the shared error/output contract
* added human, JSON, NDJSON, and quiet rendering for agent registry outputs with stable machine-readable kinds
* added transactional agent rename preserving registry fields and rolling back on failure
* extended `missive-store` with non-secret auth-ref repository structs and CRUD methods so agent `auth_ref_name` foreign keys can be satisfied without storing raw tokens
* updated CLI help snapshots and added `crates/missive-cli/tests/agent_registry.rs`
* updated README and docs for CLI usage, configuration/state behavior, storage APIs, and architecture notes
...
A2A Agent Card discovery, `missive agent inspect`, `missive agent refresh`, interface negotiation, protocol type integration, authentication material resolution, and outbound A2A calls remain for later tickets.

Agent registry commands persist config auth refs as non-secret environment/keyring references and can link local agents to config-defined auth refs, but they do not resolve, send, or validate actual secret values. There are no standalone auth-ref management commands yet.

Config-seeded agents are synced into SQLite as read-only rows when agent registry commands run. If a config entry is later removed, an already-synced row may remain in the local database until a future reconciliation/maintenance command defines stale config-seed pruning.

The `missive` binary has a real command tree, global parser, configuration discovery/profile validation, output rendering contract, and implemented agent registry commands, but other command behaviour is still intentionally skeletal. Real A2A integration, gateway behaviour, adapters, collectives, shell completion generation, and manpage generation remain for later tickets.

The store layer resolves state paths, provides process locks, migrates fresh SQLite databases to schema version 1, and exposes typed repository APIs for auth refs, agents, contexts, tasks, events, groups, group members, and gateway jobs. Message, artifact, push-config, and adapter-binding repositories, retention enforcement, compaction, event replay, and durable A2A protocol persistence behaviour remain for later tickets.
...
Ticket 014 — Implement public Agent Card discovery.
